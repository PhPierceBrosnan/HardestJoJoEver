...
void update(GameManager manager){
    ...
    if(manager.contador == 100){
        if(manager.player.points > 0){
            manager.player.points = manager.player.points - 10;
            manager.contador = 0;
        }
    }
    else{
...
void checaContato(GameManager *manager){
    ...
    if((*manager).matrizJogo[conversaoGraficoMatriz((*manager).player.posX)][conversaoGraficoMatriz((*manager).player.posY)] == 'V'){
        levelStart((*manager).level, (*manager).player.points - 10);
    }
