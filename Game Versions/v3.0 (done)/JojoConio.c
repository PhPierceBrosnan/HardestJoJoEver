...
void checaContato(GameManager *manager){
    ...
    if((*manager).matrizJogo[conversaoGraficoMatriz((*manager).player.posX)][conversaoGraficoMatriz((*manager).player.posY)] == 'V'
       || (*manager).matrizJogo[conversaoGraficoMatriz((*manager).player.posX)][conversaoGraficoMatriz((*manager).player.posY)] == 'H'){
...
// Imprime na tela informações gerais
void printaInfo(Player player, int level){
...
void moveEnemies(GameManager *manager){
    ...
    for(int j = 0; j < (*manager).contaInimigos;j++){
        ...
        if((*manager).enemies[j].direction == 'H'){
            ...
            paintPosicao(' ', 0, (*manager).enemies[j].posX - (*manager).enemies[j].vetor, (*manager).enemies[j].posY);
            (*manager).matrizJogo[conversaoGraficoMatriz((*manager).enemies[j].posX)][conversaoGraficoMatriz((*manager).enemies[j].posY)] = 'H';
            (*manager).matrizJogo[conversaoGraficoMatriz((*manager).enemies[j].posX  - (*manager).enemies[j].vetor)][conversaoGraficoMatriz((*manager).enemies[j].posY)] = '-';
